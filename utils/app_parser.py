...
class AppParser:
    ...
    @property
    @memoize
    def reverse_commits(self):
        return self._get_commits(reverse=True)

    def _get_commits(self, reverse=False):
        # Gets commit history from 'git log'

        destination_path = f"{self._app_code_dir}/.git"
        cmd = ["git", "--git-dir", destination_path, "log"]

        if reverse is True:
            cmd.append("--reverse")
        commits = get_command_output(cmd)
    ...
    @property
    @memoize
    def ship_with_source(self):
        try:
            return self.is_cross_compatible or self._get_from_json("main_module").endswith(".py")
    ...
    def python_version(self):
        return str(self.app_json.get("python_version", "3.8"))
    ...
    @property
    def is_py3(self):
        return self.python_version.startswith("3")

    @property
    @memoize
    def is_cross_compatible(self):
        return self.is_py3 and self.min_phantom_version < 3.8
