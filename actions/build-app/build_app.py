...
# Ensure the repo root is in sys.path to import utils
...
class AppBuilder:
    ...
    def _compile_app(self, ship_with_source):
        """
        Compile the app directory
        Also, remove the source code from the compiled directory if necessary
        """
        compile_cmd = f"python3 -m compileall -q -f {self.app_code_dir}"
        log("Compiling app with python3")
        run_command(compile_cmd)

        # Remove the source code if necessary
        if not ship_with_source:
            log("Removing python source code")
            for filename in os.listdir(self.app_code_dir):
                if filename.endswith(".py"):
                    os.remove(os.path.join(self.app_code_dir, filename))
